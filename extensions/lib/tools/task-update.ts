...
const executeTaskUpdate = async (
	...
	const idx = store.state.tasks.findIndex((t) => t.id === args.id);
	if (idx === -1) return failed(REFUSAL.taskNotFound(args.id), "not_found");
	const next = applyTaskPatch(store.state.tasks[idx]!, args.patch);
	const tasks = [...store.state.tasks];
	tasks[idx] = next;
	const ptrs = advancePointers(
		tasks,
		args.id,
		next,
...
export const registerTaskUpdateTool = (pi: ExtensionAPI, store: Store): void => {
	pi.registerTool({
