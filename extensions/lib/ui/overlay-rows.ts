...
const row = (theme: Theme, label: string, value: string, width: number): string =>
	truncateToWidth(`  ${theme.fg("muted", `${label}:`)}        ${value}`, width);

export const coreRows = (s: GoalState, theme: Theme, width: number): string[] => [
...
export const optionalRows = (s: GoalState, theme: Theme, width: number): string[] => {
	const out: string[] = [];
	...
	if (s.askBefore.length) out.push(row(theme, r.askBefore, s.askBefore.join(", "), width));
	if (s.decisionStyle) out.push(row(theme, r.decisions, s.decisionStyle, width));
	if (s.lastReason) {
...
export const evidenceRows = (s: GoalState, theme: Theme, width: number): string[] => {
	if (!s.evidence.length) return [];
	const out = [truncateToWidth(`  ${theme.fg("muted", OVERLAY.evidenceHeader)}`, width)];
