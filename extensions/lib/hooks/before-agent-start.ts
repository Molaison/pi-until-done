...
import { MISE_CLI_BLOCK, REMINDER, VERIFIABILITY_BLOCK } from "../strings";
import { STRUCTURAL_CONSTRAINTS_BLOCK } from "../structural-constraints";
import type { GoalState, NorthStar } from "../types";

const northStarBlock = (ns: NorthStar | undefined, fallbackGoal: string): string => {
...
const taskBlock = (s: GoalState): string => {
	...
	return REMINDER.tasksLine(done, total, current?.id ?? "(none)", current?.title ?? "");
};
...
const buildReminder = (s: GoalState): string => {
	const meta = REMINDER.phaseLine(s.phase) + REMINDER.budgetLine(s.turnsUsed, s.maxTurns);
	return REMINDER.northStarHeader + northStarBlock(s.northStar, s.goal) + meta + taskBlock(s) + STATIC_REMINDER;
};

export const registerBeforeAgentStart = (pi: ExtensionAPI, store: Store): void => {
