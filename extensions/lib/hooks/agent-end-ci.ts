...
const notifyHeadline = (ctx: ExtensionContext, headline: string, ok: boolean): void => {
...
export const runCiOnStop = async (
	...
	if (store.codeEditsThisTurn === 0) return { ranAnything: false, failed: false };
	const summary = await runCi(ctx.cwd);
	if (summary.results.length === 0) return { ranAnything: false, failed: false };
