...
const cleanEndPrompt = (ns: NorthStar | undefined): string =>
	[
		...
		"",
		ns?.verifyCommand ? CLEAN_END.withVerify(ns.verifyCommand) : CLEAN_END.withoutVerify,
		CLEAN_END.residual,
		"",
...
export const handleCleanEnd = (
	...
	pi.sendUserMessage(cleanEndPrompt(store.state.northStar), { deliverAs: "followUp" });
	refreshStatus(store, ctx);
	refreshWidget(store, ctx, true);
};
