...
const matchesAskBefore = (askBefore: string[], cmd: string): string | undefined => {
...
const handleBash = async (
	...
	const hit = matchesAskBefore(store.state.askBefore, event.input.command ?? "");
	if (hit && ctx.hasUI) {
...
export const registerToolHooks = (pi: ExtensionAPI, store: Store): void => {
	pi.on("tool_call", async (event, ctx) => {
		...
		if (isToolCallEventType("bash", event)) return handleBash(store, event, ctx);
